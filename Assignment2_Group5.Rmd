...
```
As can be seen, the sleep quality levels of 4 and 5 are not predicted in the tree. Besides, a sleep quality of 9 is predicted correctly in all cases. And the sleep quality of 7 is in most cases bad predicted (for instance in 43% of the cases, a 4 is the true data, but classified as a 7). Moreover, the tree does not use all variables; only Stress.Level, Occupation, and Sleep.Duration are used (3 out of 11 variables). This means that these variables are the strongest contributors to predicting. Quality.of.Sleep. 

```{r}
tree_full_mod <- rpart(Quality.of.Sleep ~., sleepdatatree, control = rpart.control(minbucket = 1, cp = 0))
rpart.plot(tree_full_mod)
```

By letting the splits continue untill all cases are classified, we generate the above tree. However, this model probably has too much variance to perform well on new samples (imposing a bias/variance tradeoff). 

### Performance evaluation
Now, we can use the model to predict the sleep quality of the test data.
...
The next step is to try to improve the model. We will try to improve the model by tuning the parameters. Gini and information are two different ways to measure the impurity of the nodes.
```{r}
# Model accuracy
accuracy(actual = test_data$Quality.of.Sleep, predicted = class_predicted) # Outcome: 0.8723404

# Model training based on gini-splitting
tree_mod_gini <- rpart(Quality.of.Sleep ~ .,
                       data = train_data, 
                       method = "class", 
                       parms = list(split = "gini")) # split = gini means that the split is based on the impurity of the nodes.
...
# Class predictions with gini-based splitting
pred <- predict(tree_mod_gini,
                newdata = test_data, 
                type = "class")
...
accuracy(actual = test_data$Quality.of.Sleep, predicted = pred) # Outcome is 0.8723404

accuracy(actual = test_data$Quality.of.Sleep, predicted = pred2) # Outcome is 0.9148936

rpart.plot(x = tree_mod_gini, yesno = 2, extra = 0, type = 0)
```

The accuracy of the model based on gini-splitting is 0.8723, and the accuracy of the model based on information-splitting is 0.9149. The model based on information-splitting is better at predicting the sleep quality of the test data.

### Tree pruning 
The next step is to prune the tree. Pruning is a technique in machine learning that reduces the size of decision trees by removing sections of the tree that provide little power to classify instances. Pruning reduces the complexity of the final classifier, and hence improves predictive accuracy by the reduction of overfitting.

```{r}
plotcp(tree_mod_gini)
print(tree_mod_gini$cptable)

index <- which.min(tree_mod_gini$cptable[,"xerror"]) # Outcome: 4 4
cp_optimal <- tree_mod_gini$cptable[index, "CP"] # Outcome: 0.02051282

tree_mod_pruned <- prune(tree_mod_gini, cp = cp_optimal)

# Plot optimized model
rpart.plot(tree_mod_pruned, yesno = 2, extra = 0, type = 0)
```
```{r}
pred3 <- predict(tree_mod_pruned,
                 newdata = test_data, 
                 type = "class")

accuracy(actual = test_data$Quality.of.Sleep, predicted = pred3) # Outcome is 0.8510638
```
The accuracy of the pruned model is 0.8511. This is lower than the accuracy of the model based on information-splitting.
...
Decision trees might not always effectively prioritize and select the most important features, as they work top-down and greedy. Random Forest and boosting methods provide a more reliable measure of feature importance by considering the collective contribution of features across multiple trees.
...
```
The most important predictors for the random forest model are sleep duration and stress level. This is different (or not?) than the predictors used in the single decision tree.
...
```
The performance of the random forest model is very good, with an accuracy of 97% on the test data.
ROC/AUC?
...
```
The boosting model is only slightly better than the Random Forest model. The difference is so small that it is probably just due to random chance.
